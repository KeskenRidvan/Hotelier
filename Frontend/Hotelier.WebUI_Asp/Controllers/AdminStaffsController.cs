...
public class AdminStaffsController : Controller
{
    ...
    [HttpPost]
    public async Task<IActionResult> Add(StaffAddDto staffAdd)
    {
        if (!ModelState.IsValid)
            return View();

        var client = _httpClientFactory.CreateClient();
        var jsonData = JsonConvert.SerializeObject(staffAdd);
    ...
    [HttpPost]
    public async Task<IActionResult> Update(int id, StaffUpdateDto staffUpdate)
    {
        if (!ModelState.IsValid)
            return View();
